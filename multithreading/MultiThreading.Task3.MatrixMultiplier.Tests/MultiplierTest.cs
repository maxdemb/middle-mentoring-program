...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            // let's run it a few times to ensure that result is similar all the times
            int amountOfRuns = 100;
            int[] sizes = new int[amountOfRuns];
            for(int j = 0; j < amountOfRuns; j++)
            {
                long parallelMs = 0;
                long regularMs = 0;
                int i = 2;
                do
                {
                    var m1 = new Matrix(i, i);
                    var m2 = new Matrix(i, i);

                    parallelMs = elapsedTimeMs(m1, m2, parallelMultiplier);
                    regularMs = elapsedTimeMs(m1, m2, regularMultiplier);
                    i++;
                } while (parallelMs > regularMs);
                sizes[j] = i;
            }
            ...
            Assert.IsTrue(true); // What should be here? IDK
        }

        #region private methods

        long elapsedTimeMs(Matrix m1, Matrix m2, IMatricesMultiplier multiplier)
        {
